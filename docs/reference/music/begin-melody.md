...
There are built-in melodies that you can choose from the ``||start melody||`` block. These are already composed for you and are easy to use by just selecting the one you want. If you want to play your own melody, you can [compose](/makecode-blockeditor/reference/music/making-melodies) one and use it instead of one of the built-in ones.

Melodies are a sequence of notes, each played for some small amount time, one after the other. The notes in a melody are held in an [array](/types/array) of [strings](/types/string). Each string in the array is a note of the melody. You make a melody by assembling the notes along with the _duration_ that the note plays for. The melody is [formed](/makecode-blockeditor/reference/music/making-melodies) like this:
...
You can set options for how you want the melody to play. You can ask that the melody plays just one time, ``once``, or have it keep repeating, ``forever``. With these options the melody will play in the foreground either once or continue to repeat. Of course, if you set ``forever``, any melody that was started in background will never play unless you [stop](/makecode-blockeditor/reference/music/stop-melody) the foreground melody. To make a background melody, set the option to ``once in background`` or ``forever in background``.
...
[stop melody](/makecode-blockeditor/reference/music/stop-melody), [play tone](/makecode-blockeditor/reference/music/play-tone),
[rest](/makecode-blockeditor/reference/music/rest), [ring tone](/makecode-blockeditor/reference/music/ring-tone),
...
[Making Melodies](/makecode-blockeditor/reference/music/making-melodies)
